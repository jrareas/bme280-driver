...
#ifdef DEBUG
	char message[300];
    sprintf(message, "Calibration data from read_humidity -> dig_H1:%04x;dig_H2:%04x;dig_H3:%04x;dig_H4:%04x;dig_H5:%04x;dig_H6:%04x\n", dig_H1, dig_H2, dig_H3, dig_H4, dig_H5, dig_H6);
...
#ifdef DEBUG
    sprintf(message, "Just read address: %04x identified by reg_data1[%d]; value %04x", DATA1_START + i, i, data_read);
    pr_info("%s \n", message);
#endif
    i++;
...
#ifdef DEBUG
    sprintf(message, "Just read address: %04x identified by reg_data2[%d]; value %04x", DATA2_START + i, i, data_read);
    pr_info("%s \n", message);
#endif
    i++;
  }
...
//  dig_H4 = dig_h4_msb | dig_h4_lsb;
  dig_H4 = (reg_data2[3] << 4) | (reg_data2[4] & 0x0F);
//  dig_h5_msb = (int16_t)(int8_t)reg_data2[5] * 16;
//  dig_h5_lsb = (int16_t)((reg_data2[4] & 0xF0) >> 4);
//  dig_H5 = dig_h5_msb | dig_h5_lsb;
  dig_H5 = (reg_data2[5] << 4) | (reg_data2[4] >> 4);
  dig_H6 = reg_data2[6];
#ifdef DEBUG
    sprintf(message, "Calibration data on read_calibration -> dig_H1:%04x;dig_H2:%04x;dig_H3:%04x;dig_H4:%04x;dig_H5:%04x;dig_H6:%04x\n", dig_H1, dig_H2, dig_H3, dig_H4, dig_H5, dig_H6);
//    sprintf(message, "here");
    pr_info("%s \n", message);
#endif
...
#ifdef DEBUG
  strcat(retStr, "dig_H1");
  sprintf(humidityStr, "%04x", dig_H1);
  strcat(retStr, humidityStr);
  strcat(retStr, "dig_H2");
  sprintf(humidityStr, "%04x", dig_H2);
  strcat(retStr, humidityStr);
  strcat(retStr, "dig_H3");
  sprintf(humidityStr, "%04x", dig_H3);
  strcat(retStr, humidityStr);
  strcat(retStr, "dig_H4");
  sprintf(humidityStr, "%04x", dig_H4);
  strcat(retStr, humidityStr);
  strcat(retStr, "dig_H5");
  sprintf(humidityStr, "%04x", dig_H5);
  strcat(retStr, "dig_H6");
  sprintf(humidityStr, "%04x", dig_H6);
#endif
  strcat(retStr, "\n");
...
static int init_driver(void)
{
  ...
  // Create class
  dev_class = class_create((char *)THIS_MODULE);
  if (IS_ERR(dev_class))
  {
